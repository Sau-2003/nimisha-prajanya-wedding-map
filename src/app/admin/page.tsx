...
import { MapPin, Users, Phone, Plus, Save, Home, Car, Trash2, RefreshCw, Star, CheckCircle } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [gRoomName, setGRoomName] = useState(""); const [gRoomLat, setGRoomLat] = useState("15.385"); const [gRoomLng, setGRoomLng] = useState("73.840");
  const [sheetUrl, setSheetUrl] = useState(''); const [syncStatus, setSyncStatus] = useState<string | null>(null);
  const [locName, setLocName] = useState(""); const [locType, setLocType] = useState("Function"); const [locLat, setLocLat] = useState("15.385"); const [locLng, setLocLng] = useState("73.840");
  ...
  // ==========================================
  // LOCATIONS LOGIC (WITH ACTIVE TOGGLE)
  // ==========================================
  const handleAddLocation = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('locations').insert([{
      ...
      is_active: isFirstFunction // Auto-make active if it's the first one added
    }]);
      ...
      complete: async function(results) {
        ...
        const mappedData: any[] = results.data.map((row: any) => {
          ...
          return {
            ...
            staying: true,
            room_name: hotel && room ? `${hotel} - Room ${room}` : (room || hotel || 'Unassigned'),
        ...
        setSyncStatus(`Successfully grabbed and saved ${mappedData.length} live guests!`);
        setSheetUrl(''); fetchAllData();
      },
      error: () => setSyncStatus("Sync failed. Is the sheet set to 'Anyone with the link can view'?")
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="bg-emerald-700 text-white p-6 shadow-md rounded-b-3xl mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-widest mt-2">SOPAGO ADMIN</h1>
          <p className="text-emerald-200 text-xs mt-1">Logged in as {adminEmail}</p>
        </div>
        <button onClick={() => setIsAdminLoggedIn(false)} className="text-xs bg-emerald-800 px-3 py-1 rounded-full font-bold">Logout</button>
      </div>
      ...
      <div className="px-4 max-w-md mx-auto">
          ...
          <div className="space-y-6 animate-fade-in">
            <AdminCard title="Add Map Location">
              <form onSubmit={handleAddLocation} className="space-y-4">
                <input type="text" value={locName} onChange={e => setLocName(e.target.value)} required placeholder="e.g. Haldi Venue" className="w-full bg-gray-100 p-3 rounded-xl outline-none text-sm font-bold" />
                ...
                <button type="submit" className="bg-emerald-600 text-white font-bold py-4 rounded-xl w-full flex justify-center items-center gap-2 shadow-lg">
                  <Plus size={18} /> Drop Pin on Map
                </button>
              </form>
            </AdminCard>
            ...
            <AdminCard title={`Saved Map Pins (${locationList.length})`}>
              <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                {locationList.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No locations added.</p>}
                ...
                {locationList.map((loc) => (
                  <div key={loc.id} className={`p-3 rounded-lg border flex flex-col gap-3 text-sm ${loc.is_active ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-bold text-gray-800 flex items-center gap-1">
                          {loc.name} {loc.type === 'washroom' ? '🚻' : loc.type === 'reception' ? '🎉' : '📍'}
                          {loc.is_active && <Star size={14} className="text-yellow-500 fill-yellow-500 ml-1" />}
                        </p>
                    ...
                    {/* Show "Set Active" button only for Functions */}
                    {loc.type === 'function' && (
                      <button 
                        onClick={() => handleSetActiveFunction(loc.id)}
                        disabled={loc.is_active}
                        className={`py-2 rounded-lg font-bold text-xs flex justify-center items-center gap-2 ${loc.is_active ? 'bg-emerald-200 text-emerald-800 cursor-not-allowed' : 'bg-white border border-gray-300 text-gray-600 shadow-sm'}`}
                      >
        ...
        {activeTab === 'guests' && (
          <div className="space-y-6 animate-fade-in">
            {/* Same as before... removed for brevity in this block, refer to previous code structure */}
            ...
            <AdminCard title={`Guest Database (${guestList.length})`}>
              <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                {guestList.map((guest) => (
                  <div key={guest.id} className="bg-gray-50 p-3 rounded-lg border flex justify-between items-center text-sm">
                    <div>
                      <p className="font-bold text-gray-800">{guest.name}</p>
                      {guest.staying && <p className="text-[10px] font-bold text-emerald-700 mt-1">🏠 {guest.room_name}</p>}
                    </div>
                    <div className="flex items-center gap-3">
                      <button onClick={() => handleDeleteGuest(guest.id)} className="text-red-400 p-2"><Trash2 size={16}/></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <AdminCard title={`Active Contacts (${helpList.length})`}>
              <div className="space-y-2">
                {helpList.map((help) => (
                  <div key={help.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <div>
                      <p className="font-bold text-gray-800 text-sm">{help.name}</p>
                      <p className="text-xs font-mono text-emerald-700">{help.phone}</p>
                    </div>
                    <button onClick={() => handleDeleteHelp(help.id)} className="text-red-400 p-2"><Trash2 size={16}/></button>
                  </div>
                ))}
              </div>
            </AdminCard>
